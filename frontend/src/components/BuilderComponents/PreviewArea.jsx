...
const PreviewArea = ({ content, device, onDeviceToggle, tenantInfo, isEditing, onContentChange }) => {
        ...
        <main className="flex-grow-1 p-4 bg-body-tertiary">
            ...
            <div className="w-100 d-flex align-items-center justify-content-between mb-4">
                ...
                <div className="device-frame-options d-flex gap-2">
                    <button
                        className={`btn ${device === 'desktop' ? 'btn-primary' : 'btn-outline-primary'}`}
                        onClick={() => onDeviceToggle('desktop')}
                    >
                        Desktop
                    </button>
                    <button
                        className={`btn ${device === 'mobile' ? 'btn-primary' : 'btn-outline-primary'}`}
                        ...
                        {isEditing ? (
                            ...
                            >
                                {localContent.blocks && localContent.blocks.length > 0 ? (
                                    ...
                                    <div className="d-flex align-items-center justify-content-center" style={{ height: '100%', minHeight: 400 }}>
                                        <div className="text-center text-muted">
                                            <h4>No components yet</h4>
                                            <p>Drag components from the sidebar to start building your storefront</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                        ) : (
                            <StaticPreview content={localContent} />
                        )}
                    </div>
                </div>
            </div>
        ...
        return (
            <div className="d-flex align-items-center justify-content-center" style={{ height: '100%' }}>
                <div className="text-center">
                    ...
                    <p className="text-muted">Enter edit mode to start building your storefront</p>
                </div>
            </div>
        );
    }
